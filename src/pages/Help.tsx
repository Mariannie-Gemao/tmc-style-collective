...
const supportOptions = [
  { icon: MessageCircle, title: "Viber Chat", desc: "Chat with our support team via Viber", action: "Open Viber", gradient: "from-purple-500 to-violet-600", lightBg: "bg-purple-500/10", textColor: "text-purple-500" },
  { icon: Phone, title: "Call Us", desc: "+63 912 345 6789 (Mon-Sat, 9AM-6PM)", action: "Call Now", gradient: "from-emerald-500 to-teal-600", lightBg: "bg-emerald-500/10", textColor: "text-emerald-500" },
  { icon: Mail, title: "Email Support", desc: "support@tmcshop.com — response within 24hrs", action: "Send Email", gradient: "from-blue-500 to-indigo-600", lightBg: "bg-blue-500/10", textColor: "text-blue-500" },
];
        ...
        <div className="container py-8 max-w-4xl">
            ...
            {supportOptions.map((opt, i) => (
              ...
              >
                ...
                <div className="p-6 text-center">
                  <div className={`inline-flex p-4 rounded-2xl ${opt.lightBg} mb-4 group-hover:scale-110 transition-transform duration-300`}>
                    <opt.icon className={`h-7 w-7 ${opt.textColor}`} />
                  </div>
                  ...
                  <Button className={`rounded-full px-6 bg-gradient-to-r ${opt.gradient} text-white border-0 hover:opacity-90 transition-opacity btn-shine`}>
                    {opt.action}
                  </Button>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-10">
            {helpCategories.map((cat) => (
              <div key={cat.title} className="bg-card border border-border/40 rounded-xl p-4 hover:shadow-md transition-shadow cursor-pointer group">
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <details key={i} className="bg-card border border-border/40 rounded-xl overflow-hidden group">
